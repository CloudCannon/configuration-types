...
export const MarkdownSettingsSchema = z.object({
	engine: z.enum(['commonmark', 'kramdown'])
		.default('commonmark')
	...
	options: z.object({
		html: z.boolean()
			.optional()
			.describe('Output HTML tags from source.'),
		
		xhtml: z.boolean()
			.optional()
			.describe('Use `/` to close single tags (e.g. `<br />`).'),
		...
		linkify: z.boolean()
			.optional()
			.describe('Autoconvert URL-like text to links.'),
		...
		quotes: z.string()
			.optional()
			.describe('Double + single quotes replacement pairs, when typographer enabled and smartquotes on. For example, you can use "«»„"" for Russian, ""„"‚"" for German, and ["«\\xA0", "\\xA0»", "‹\\xA0", "\\xA0›"] for French (including `nbsp`).'),
		
		spaced_lists: z.boolean()
			.optional()
			.describe('Output lists with an extra space in Markdown.'),
		
		sentence_per_line: z.boolean()
			.optional()
			.describe('Add linebreaks between sentences in Markdown.'),
		
		gfm: z.boolean()
			.optional()
			.describe('Enable GFM mode.'),
		
		code_block_fences: z.enum(['```', '~~~'])
			.optional()
			.describe('Determines which style of code block fences to use.'),
		
		treat_indentation_as_code: z.boolean()
			.optional()
			.describe('Determines whether 4 spaces on indentation should be read as a code block.'),
		
		escape_snippets_in_code_blocks: z.boolean()
			.optional()
			.describe('Render snippets as plain text within code blocks.'),
		
		table: z.boolean()
			.optional()
			.describe('Output tables in Markdown format.'),
		...
		heading_ids: z.boolean()
			.optional()
			.describe('Generate IDs for headings.'),
		
		attributes: z.boolean()
			.optional()
			.describe('Save element attributes in Markdown format instead of converting to HTML.'),
		
		attribute_elements: MarkdownAttributeElementOptionsSchema
			.optional()
			.describe('Define positioning behavior of Markdown attributes for different elements.')
	})
});
