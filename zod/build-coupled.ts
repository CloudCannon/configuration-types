...
export const BuildCoupledPathsSchema = PathsSchema.extend({
	collections: z.string().optional().describe('Parent folder of all collections.'),

	data: z.string().optional().describe('Parent folder of all site data files.'),
...
export const FilterSchema = z.object({
	base: FilterBaseSchema.optional().describe(
...
export const ParseableSchema = z.object({
	parser: z
		.enum(['csv', 'front-matter', 'json', 'properties', 'toml', 'yaml'])
		.optional()
		.describe(
...
export const FilterableSchema = z.object({
	filter: z
		.union([FilterSchema, FilterBaseSchema])
		.optional()
		.describe(
...
export const BuildCoupledConfigurationSchema = z.object({
	...
	collections_config: BuildCoupledCollectionConfigSchema.optional().describe(
		'Definitions for your collections, which are the sets of content files for your site grouped by folder. Entries are keyed by a chosen collection key, and contain configuration specific to that collection.'
	),
...
export const ParsedCollectionItemSchema = z.record(z.string(), z.unknown()).and(
	z.object({
		path: z.string().describe('The path to the file this was parsed from.'),
		...
		url: z.string().optional().describe('The URL this file is served at once built.'),
	})
);
...
export const WithIntegrationOutputSchema = z.object({
	...
	time: z.string().optional().describe('The time this file was generated.'),
	...
	data: z
		.record(z.string(), ParsedDatasetSchema)
		.optional()
		.describe('Map of data keys to parsed datasets. Keys match keys in `data_config`.'),

	collections: z
		.record(z.string(), z.array(ParsedCollectionItemSchema))
		.optional()
		.describe(
			'Map of collection keys to parsed collection files. Keys match keys in `collections_config`.'
		),

	files: z.record(z.string(), z.string()).optional().describe('Map of build file paths to MD5s.'),
});
...
export const JekyllIntegrationOutputSchema = z.object({
	...
	...WithIntegrationOutputSchema.shape,
	defaults: z.unknown().describe('@deprecated Do not use.'),
});
...
export const BuildCoupledSchema = z
	.union([
